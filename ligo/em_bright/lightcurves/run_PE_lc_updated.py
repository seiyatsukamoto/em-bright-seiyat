...
def run_lc(m1, m2, d, event):
        path = f'./mdc_analysis/updated_fits2/PE_MDC9_500/{event}'
        ...
        #path = f'./PE_mdc/PE_later_MDC/{event}'
        #path = f'./mdc_analysis/PE_1000/{event}'
...
for event in dirs:
    ...
    try:
        ...
        try:
            ...
            PE_data['mass_1'] = PE_data['mass_1_source']
            PE_data['mass_2'] = PE_data['mass_2_source']
        if downsample_PE:
        ...
        posterior_qs = False
        if posterior_qs:
            run_lc(m1, m2, d, event)

        #run_lc(m1, m2, d, event)
        #if len(RP_events_cut) > 0:
        if not posterior_qs:
            if len(RP_events_cut['mass_ratio']) > 0:
